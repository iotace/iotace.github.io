...
function createExplorerTemplate(cfg) {
  const explorerTemplate =
    `    
    <div id="explorerLogo">
      <a href="http://iotace.co.id" target="_blank" rel="noopener noreferrer"
...
class BIMViewer extends Controller {
    ...
    try {
      switch (name) {
        ...
        case "scaleCanvasResolution":
          this._fastNavPlugin.scaleCanvasResolution = this._configs[name] =
            parseBool(value);
          break;
  ...
  setObjectColorSource(source) {
    console.log(
      "BIMViewer.setObjectColorSource() is now deprecated and no longer functional. By default, " +
  ...
  getNumMeasurements() {
    return (
      this._measureDistanceTool.getNumMeasurements() +
      this._measureAngleTool.getNumMeasurements()
    );
  }
  ...
  getMeasurementsAxisVisible() {
    return this._enableMeasurements
      ? this._measureDistanceTool.getMeasurementsAxisVisible()
      : false;
  }
  ...
  getMeasurementsSnappingEnabled() {
    return this._enableMeasurements
      ? this._measureDistanceTool.getSnappingEnabled()
      : false;
  }
